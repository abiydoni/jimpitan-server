...
import { 
  Village, User, Role, Menu, ChatMessage, Slide, 
  Schedule, Tariff, Exemption, InventoryItem, InventoryLoan, 
  DuesJournal, JimpitanHistory 
} from './models';
...
const migrateData = async () => {
  try {
    ...
    const chatsSnap = await db.collection('chat_messages').get();
    let chatCount = 0;
    for (const doc of chatsSnap.docs) {
      const data = doc.data();
      // Validasi FK: Pastikan senderUid dan receiverUid ada di tabel users
      if (data.senderUid && data.receiverUid && data.villageId) {
        const senderExists = await User.findByPk(data.senderUid);
        const receiverExists = await User.findByPk(data.receiverUid);
        const villageExists = await Village.findByPk(data.villageId);
    ...
    // 7. Schedules
    console.log('\n📦 Migrasi Schedules...');
    const schedsSnap = await db.collection('schedules').get();
    let schedCount = 0;
    ...
    // 8. Tariffs
    console.log('\n📦 Migrasi Tariffs...');
    const tariffsSnap = await db.collection('tariffs').get();
    let tariffCount = 0;
    ...
    // 9. Exemptions
    console.log('\n📦 Migrasi Exemptions...');
    const exemptionsSnap = await db.collection('exemptions').get();
    let exempCount = 0;
    for (const doc of exemptionsSnap.docs) {
      const data = doc.data();
      if (data.tariffId && data.villageId && data.kkId) {
        const tariffExists = await Tariff.findByPk(data.tariffId);
        if (tariffExists) {
          await Exemption.findOrCreate({
            where: { id: doc.id },
            defaults: {
              id: doc.id,
              kkId: data.kkId,
    ...
    const loansSnap = await db.collection('inventory_loans').get();
    let loanCount = 0;
    for (const doc of loansSnap.docs) {
      const data = doc.data();
      if (data.itemId && data.villageId) {
        const itemExists = await InventoryItem.findByPk(data.itemId);
        if (itemExists) {
          await InventoryLoan.findOrCreate({
            where: { id: doc.id },
            defaults: {
              id: doc.id,
              itemId: data.itemId,
    ...
    const journalsSnap = await db.collection('dues_journals').get();
    let journalCount = 0;
    for (const doc of journalsSnap.docs) {
      const data = doc.data();
      if (data.villageId) {
        await DuesJournal.findOrCreate({
          where: { id: doc.id },
          defaults: {
            id: doc.id,
            villageId: data.villageId,
    ...
    const jimpitanSnap = await db.collection('jimpitan_history').get();
    let jimpitanCount = 0;
    for (const doc of jimpitanSnap.docs) {
      const data = doc.data();
      if (data.villageId) {
        await JimpitanHistory.findOrCreate({
          where: { id: doc.id },
          defaults: {
            id: doc.id,
            villageId: data.villageId,
