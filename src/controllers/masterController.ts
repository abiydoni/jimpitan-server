...
const generateDefaultVillageConfig = async (existingConfig?: any) => {
  ...
  const dbMenus = await Menu.findAll();
  const allMenuIds = new Set<string>(standardMenuIds);
  dbMenus.forEach((m: any) => {
    if (m && m.id) allMenuIds.add(m.id);
  });

  const menuPermissions: any = existingConfig?.menuPermissions ? JSON.parse(JSON.stringify(existingConfig.menuPermissions)) : {};
  ...
  try {
    ...
    if (deletedDocIds && Array.isArray(deletedDocIds) && deletedDocIds.length > 0) {
      ...
      if (validDeletes.length > 0) {
        ...
        const deleteWhere: any = { uid: validDeletes };
        if (villageId) {
          deleteWhere.villageId = villageId;
        }
        const toDelete = await User.findAll({ where: deleteWhere, transaction });
        console.log(`[saveUserFamily] Akan menghapus ${toDelete.length} user: ${toDelete.map((u: any) => u.getDataValue('uid')).join(', ')}`);
        if (toDelete.length > 0) {
          await User.destroy({ where: deleteWhere, transaction });
        }
      }
    }
...
export const createSlide = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const slide = await Slide.create({ id, title, subtitle, type, imageBase64, textColor, value, status, villageId });
...
export const updateSlide = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const slide = await Slide.findByPk(id as string);
    if (!slide) {
...
export const updateFcmToken = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await User.update({ fcmToken } as any, { where: { uid } });
...
export const removeFcmToken = async (req: Request, res: Response): Promise<void> => {
  try {
    const { uid } = req.params;
    await User.update({ fcmToken: null } as any, { where: { uid } });
    res.json({ success: true });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
...
export const bulkImportUsers = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    for (const u of users) {
      ...
      if (!families[code]) families[code] = [];
    ...
    for (const [code, members] of Object.entries(families)) {
      ...
      const existingUser = await User.findOne({ where: { noKK: code, villageId }, transaction });
